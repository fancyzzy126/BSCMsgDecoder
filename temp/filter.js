...
{
	var content;
	//alert("showver_proxy len:" + de.childNodes.length); 
	//alert("xml:" + (new XMLSerializer()).serializeToString(de));
    if ( de.childNodes.length > 0 )
    {
        ...
        for (var i = 0; i < de.childNodes.length; i++)
        {
			// childNodes[i] must use [], and chrome do not support ()
	        var child = de.childNodes[i];
			var newName = child.tagName;
...
{
	editbuffer(msgid, "showmsg", "decode_single_msg", msg_str)
}